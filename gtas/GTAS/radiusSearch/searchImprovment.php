...
#	@Argument		:	NA
function getEventInfo1($zipArray,$eventTypeInfo,$stDate='',$endDate='',$distanceArray, &$getResultForPrintValue)
{	  
	  $getFinalresult   = '';
 ...
 */ 
	...
	foreach ($eventTypeInfo as $key => $value)
	{
		...
		$sql= "SELECT 
                ...
                FROM 
		                 events
                WHERE 
		                 Event_ZIP IN ($getZipcode) $andEventCondition";	
				  ...
				  if(!empty($eventTypeInfo))
	                {
						$checkEventCondition = $resultArray['Event_Category']; 
							...
							$getResultForPrintValue[]=array($eventName,$eventCategory,$eventSDate,$temp);
							echo $distanceArray[$eventZip];
					}
					else
					{	
	...
	$keyArray[]=$sZipCode;
	$finalDataResult .=  getEventInfo1($keyArray,$getselectedEventType,$getStDate,$getEndDate,$a_result,$getResultForPrintValue);
